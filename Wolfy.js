...
Octopus.on('message', msg => {
  ...
  if (msg.content === 'wolfy') {
    msg.reply('""Emredin Sultanım""');
  }
  
  if (msg.content === 'wolfy nasılsın') {
   	msg.reply('**Allaha Şükür İyiyim Sen?**');
  }
  
  if (msg.content === 'wolfy iyiyim') {
   	msg.reply('**Allah İyilik Versin**');
  }
  
  if (msg.content === 'allaha emanet') {
   	msg.reply('**Görüşürüz**');
  }
  
  if (msg.content === 'w!botoyla') {
   	msg.reply('https://discordbots.org/bot/472733634760146945 Botumuzu oyladığınız için teşekkürler');
  }
...
Octopus.login(process.env.BOT_TOKEN);
